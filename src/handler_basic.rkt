...
;;```
(struct writer (file filter postprocess))
;;```
...
;;omitted. Finally the \textit{postprocess} function can do modifications to the 
;;content of a chunk before it is written into the file.
;;
;;This handler module also provides defaults that can be used for the 
;;\textit{file}, \textit{filter} or \textit{postprocess} members of a writer.
;;
...
(define (default-docu-writer-filter c)
  #t)
;;```
;;The default \textit{postprocess} method simply gives the lines from the chunk as
;;they are without any modifications.
;;```
(define (default-writer-postprocess c)
  (chunk-lines c))
;;```
...
;;postprocessed lines of the given chunk:
;;```
  (for-each write-lines
    (map (writer-postprocess w)
